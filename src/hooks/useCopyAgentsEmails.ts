import { apiPathAgent, MAX_PAGE_LIMIT } from "@/config/constants";
...
export const useCopyAgentsEmails = (serializedFilter: URLSearchParams) => {
  ...
  async function fetchEmailPage(page: number) {
    const res = await queryClient.fetchQuery({
      queryKey: ["agents-emails", serializedFilter.toString(), page],
      staleTime: 0,
      queryFn: () =>
        fetchData<ApiAgentGetList[]>(`${apiPathAgent}/`, {
    ...
    return {
      emails: res.data.map((agent) => agent.email).filter((e): e is string => Boolean(e)),
      count: res.count,
    };
  }

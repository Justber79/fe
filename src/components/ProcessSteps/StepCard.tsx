...
import { IconDiv, NumberingDiv } from "../styled/container";
...
const CardInfoBulletDiv = styled.div`
  ...
  justify-content: center;
  align-items: center;
  padding-left: var(--homepage-process-section-card-info-bullet-div-padding-left);
`;
...
export default function StepCard({ index, iconName, infoTexts, title, week }: Props) {
  const bulletDashColor = infoTexts.length === 1 ? "var(--color-magnolia)" : undefined;
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <IconDiv size="var(--homepage-process-section-card-icon-size)">{iconNameMap[iconName]}</IconDiv>
      </CardHeader>
      <CardInfoDiv>
        <CardInfoHeadLineDiv>
          <NumberingDiv>
